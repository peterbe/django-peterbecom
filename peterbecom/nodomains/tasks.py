...
COUNT_JS_PATH = os.path.join(os.path.dirname(__file__), "count.js")
...
def run_url(url, dry_run=False):
    ...
    if domains:
        if not dry_run:
            r = models.Result.objects.create(url=url, count=len(domains))
            for domain in domains:
                models.ResultDomain.objects.create(
                    result=r, domain=domain, count=domains[domain]
                )
    else:
