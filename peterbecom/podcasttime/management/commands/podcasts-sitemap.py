...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--base-url',
            default=None,
            help='E.g https://podcasttime'
        )
        parser.add_argument(
            '--outfile', '-o',
            default=None,
            help='If not site, use stdout'
        )
            ...
            urls_xmls.append(
                ...
                '</url>'.format(
                    absolute_url(
                        '/podcasts/{}/{}'.format(
                            podcast.id,
                            podcast.get_or_create_slug(),
                        )
                    ),
                    (podcast.latest_episode or podcast.modified).strftime(
                        '%Y-%m-%d'
                    ),
                )
            )
